...
import {
  LANGUAGE_CODES, languageLabel, languageLocale,
...
import conversation from "./two-language-translate.module.css";
...
const SPEAKERS: Record<Speaker, string> = { me: "自分", partner: "相手" };
...
function LanguagePicker({ label, id, value, disabled, status, onChange }: {
  ...
  return <label className={multi.picker} htmlFor={id}>
    ...
    <small>音声認識：{status}</small>
  </label>;
}

export default function TwoLanguageTranslatePage() {
  ...
  const [status, setStatus] = useState("自分と相手の言語を選び、「開始」を押してください。");
  ...
  // The two participant languages are the only output languages. There is no
  // separate three-language picker or single-input language setting.
  const outputs = useMemo<Lang[]>(() => [...new Set([myLanguage, partnerLanguage])], [myLanguage, partnerLanguage]);
  const selectedSources = useMemo<Lang[]>(() => mode === "both" ? [myLanguage, partnerLanguage] :
    [mode === "mic" ? myLanguage : partnerLanguage], [mode, myLanguage, partnerLanguage]);

  // Advisory only: actual recognition and translation depend on the user's Chrome build.
  useEffect(() => {
    let cancelled = false;
  ...
  const stop = useCallback(() => {
    ...
    setStatus("停止しました。会話履歴はコピー・CSV保存できます。");
  }, [release]);
  ...
  const begin = useCallback(async () => {
    ...
    if (selectedSources.some((code) => support[`speech:${code}`] === "unavailable")) {
      setError("選択した言語が、このブラウザの音声認識では利用できません。言語を変更してください。");
      return;
    }
    ...
    setBusy(true);
    // Prime translation inside the initial user gesture, before permission prompts.
    for (const from of new Set(selectedSources)) for (const to of outputs) if (from !== to) primeRoute(from, to);
    try {
      const browser = window as RecognitionWindow;
      ...
      if (pc) startRecognizer("partner", pc, partnerLanguage, [...outputs], sessionId);
      if (mic) startRecognizer("me", mic, myLanguage, [...outputs], sessionId);
      setRunning(true);
      setStatus("自分と相手の発言を、それぞれの言語で２つの窓に表示しています。");
    } catch (caught) {
      release();
      setRunning(false);
  ...
  }, [busy, mode, myLanguage, partnerLanguage, outputs, selectedSources, support, startRecognizer, release, stop]);
  ...
  const downloadCsv = () => {
    ...
    anchor.href = url;
    anchor.download = `digil-cloud-two-language-${new Date().toISOString().slice(0, 10)}.csv`;
    anchor.click();
    window.setTimeout(() => URL.revokeObjectURL(url), 1000);
  };
  const sessionEntries = entries.filter((entry) => entry.sessionId === activeSessionId);
  const recentEntries = [...sessionEntries].reverse();
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.hero}>
        <p className={styles.eyebrow}>TWO-WAY LIVE INTERPRETATION</p>
        <h1>自分と相手、２つの言語でリアルタイム通訳</h1>
        <p>自分と相手の言語を選ぶだけ。２つの窓に、会話をそれぞれが読める言語で表示します。</p>
      </section>
      <section className={styles.panel}>
        ...
        <div className={multi.pickers}>
          <LanguagePicker label="自分の言語（マイク）" id="my-language" value={myLanguage} disabled={busy || running} status={speechStatus(myLanguage)} onChange={(code) => changeLanguage(setMyLanguage, code)} />
          <LanguagePicker label="相手の言語（PC音声）" id="partner-language" value={partnerLanguage} disabled={busy || running} status={speechStatus(partnerLanguage)} onChange={(code) => changeLanguage(setPartnerLanguage, code)} />
        </div>
        ...
        <p className={dual.note}>入力元ごとに聞き取り言語は１つです。途中で別の言語に切り替える場合は、停止して言語を変更してください。声紋による発言者判定ではありません。</p>
        ...
        <div className={styles.help}>PC音声は共有画面で「タブの音声を共有」または「システム音声を共有」をONにしてください。音の回り込みを防ぐためヘッドホンを推奨します。録音・文字起こしは参加者の同意を得て行ってください。音声ファイルは保存せず、文字起こしを画面内に保持します。音声認識にはブラウザ提供のサービスが使われる場合があります。</div>
      </section>
      <section className={styles.panel}>
        <div className={styles.logHeader}><div><p className={styles.eyebrow}>LIVE INTERPRETATION</p><h2>自分と相手の会話</h2></div><span>{running ? "● 認識中" : "待機中"}</span></div>
        <div className={dual.columns} aria-live="polite">
          {(["me", "partner"] as const).map((viewer) => {
            const language = viewer === "me" ? myLanguage : partnerLanguage;
            return <article key={viewer} className={dual.channel} aria-label={`${SPEAKERS[viewer]}の画面・${languageLabel(language)}`}>
              <p className={dual.speaker}>{SPEAKERS[viewer]}の画面</p>
              <p className={dual.direction}>{languageLabel(language)}で表示</p>
              <div className={conversation.history}>
                {recentEntries.length === 0 && !interim.me && !interim.partner ? <p className={conversation.empty}>発言を待っています…</p> : null}
                {interim.me || interim.partner ? (["me", "partner"] as const).map((speaker) => interim[speaker] ? <div key={`interim-${speaker}`} className={conversation.utterance}>
                  <span className={conversation.speaker}>{SPEAKERS[speaker]}・認識中</span>
                  <p className={conversation.text} dir="auto">{(speaker === "me" ? myLanguage : partnerLanguage) === language ? interim[speaker] : "発言を認識しています…"}</p>
                </div> : null) : null}
                {recentEntries.map((entry) => <div key={entry.id} className={conversation.utterance}>
                  <span className={conversation.speaker}>{SPEAKERS[entry.speaker]} <time>{timeOf(entry.timestamp)}</time></span>
                  <p className={conversation.text} dir="auto">{entry.translations[language] ?? (entry.source === language ? entry.original : "翻訳中…")}</p>
                </div>)}
              </div>
              {channelErrors[viewer] ? <p className={dual.channelError} role="alert">{channelErrors[viewer]}</p> : null}
            </article>;
          })}
        </div>
      </section>
      <section className={styles.panel}>
        <div className={styles.logHeader}><div><p className={styles.eyebrow}>TRANSCRIPT</p><h2>発言者付き会話履歴</h2></div><span>{entries.length}件</span></div>
