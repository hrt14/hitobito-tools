...
export default function Breakthrough90() {
  ...
  const finish = () => {
    ...
    try {
      ...
      // Saving is a convenience, not a requirement for the core experience.
    }
      ...
      <main className={styles.shell}>
        ...
        <section className={styles.intro}>
          ...
          <p className={styles.lead}>
            問題を消すアプリではありません。90秒で視点を引き、比較対象を戻し、いま動かせる一手だけを決めます。
          </p>
          ...
          <div className={styles.inputPanel}>
            ...
            <div className={styles.pressureRow}>
              <div>
                <span className={styles.fieldLabel}>いまの詰まり感</span>
                <span className={styles.pressureHint}>直感でOK</span>
              </div>
            ...
            <input
              ...
              aria-label="いまの詰まり感"
              onChange={(event) => setPressureBefore(Number(event.target.value))}
            />
          ...
          {lastSession && (
            <aside className={styles.lastSession}>
              <span>前回の一手</span>
              <strong>{lastSession.nextAction}</strong>
            </aside>
          )}
        </section>
      </main>
      ...
      {stage === "zoom" && (
        <section className={styles.stage}>
          ...
          <div className={styles.zoomField}>
            ...
            <input
              className={`${styles.range} ${styles.horizonRange}`}
              type="range"
              min="0"
              max="10"
              value={horizon}
              aria-label="時間軸を10年まで広げる"
              onChange={(event) => setHorizon(Number(event.target.value))}
            />
          </div>

          <div className={styles.insightBox}>
            <span>10年後から見る問い</span>
            <strong>「これは人生の結論か。それとも途中の一局面か？」</strong>
          </div>
      ...
      {stage === "compare" && (
        <section className={styles.stage}>
          ...
          <div className={styles.compareBoard}>
            {comparisons.map((item) => {
              ...
              return (
                <button
                  key={item}
                  className={`${styles.compareChip} ${isDiscarded ? styles.discarded : ""}`}
                  type="button"
                  disabled={isDiscarded}
                  onClick={() => discardComparison(item)}
                >
                  <span>{item}</span>
                  <small>{isDiscarded ? "比較対象から外した" : "タップして外す"}</small>
                </button>
              );
            })}
            <div className={`${styles.selfTarget} ${allComparisonsDiscarded ? styles.selfTargetReady : ""}`}>
              <span>比較対象</span>
              <strong>{allComparisonsDiscarded ? "昨日の自分" : "？"}</strong>
            </div>
          </div>
          ...
          {allComparisonsDiscarded && (
            <div className={styles.deltaPanel}>
              ...
              <input
                ...
                maxLength={90}
                placeholder="例：原因の仮説を1つ捨てられた"
                onChange={(event) => setYesterdayDelta(event.target.value)}
              />
              <button
                className={styles.ghostButton}
                type="button"
                onClick={() => setYesterdayDelta("まだ見つからない。だから今日の一手を1mmにする")}
              >
                まだ見つからない
              </button>
            </div>
          ...
          <button
            className={styles.primaryButton}
            type="button"
            disabled={!allComparisonsDiscarded || yesterdayDelta.trim().length < 2}
            onClick={() => {
              setStage("action");
              buzz();
            }}
      ...
      {stage === "action" && (
        <section className={styles.stage}>
          ...
          <p className={styles.stageLead}>
            自信を気分として待ちません。最終的には抜ける前提なら、次の15分で何をするかだけ決めます。
          </p>

          <div className={styles.problemReminder}>
            <span>いまの問題</span>
          ...
          <div className={styles.actionGrid} role="group" aria-label="次の一手の種類">
            {actionTypes.map((item) => (
              <button
                key={item.label}
                type="button"
                className={`${styles.actionType} ${actionType === item.label ? styles.actionTypeActive : ""}`}
                onClick={() => chooseActionType(item.label)}
              >
                {item.label}
              </button>
            ))}
          ...
          {actionType && (
            <div className={styles.actionDetail}>
              <label className={styles.fieldLabel} htmlFor="next-action">次の15分でやること</label>
              <input
                id="next-action"
                className={styles.textInput}
                value={nextAction}
                maxLength={100}
                placeholder={actionTypes.find((item) => item.label === actionType)?.hint}
          ...
          >
            ...
            <span className={styles.holdText}>{isHolding ? "そのまま長押し" : "長押しして、この一手に決める"}</span>
          </button>
        </section>
      )}
      ...
      {stage === "rate" && (
        <section className={`${styles.stage} ${styles.rateStage}`}>
          ...
          <div className={styles.pressureMeter}>
            <span>開始 {pressureBefore}/10</span>
            <strong>{pressureAfter}/10</strong>
            <span>現在</span>
          </div>
          <input
            className={`${styles.range} ${styles.finalRange}`}
            type="range"
            min="1"
            max="10"
            value={pressureAfter}
            aria-label="現在の詰まり感"
            onChange={(event) => setPressureAfter(Number(event.target.value))}
          />

          <div className={styles.finalActionPreview}>
            <span>次の15分</span>
            <strong>{nextAction}</strong>
          </div>

          <button className={styles.primaryButton} type="button" onClick={finish}>
            突破カードを作る
          </button>
        </section>
      )}
        ...
        <section className={`${styles.stage} ${styles.doneStage}`}>
          <div className={styles.resultCard}>
            <div className={styles.resultHead}>
              <span>BREAKTHROUGH CARD</span>
              <strong>{pressureBefore}/10 → {pressureAfter}/10</strong>
            </div>
            ...
            <div className={styles.resultRows}>
              ...
              <div>
                <span>昨日との差分</span>
                <strong>{yesterdayDelta}</strong>
              </div>
              <div className={styles.resultAction}>
                <span>次の15分 / {actionType}</span>
                <strong>{nextAction}</strong>
              </div>
            </div>
            ...
            <div className={styles.resultFoot}>
              {pressureChange > 0 ? (
                <strong>詰まり感 −{pressureChange}</strong>
              ) : pressureChange < 0 ? (
                <strong>詰まり感 +{Math.abs(pressureChange)}</strong>
              ) : (
                <strong>詰まり感は同じ</strong>
              )}
          ...
          <div className={styles.doneActions}>
            <a className={styles.primaryButton} href="#top" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>
              15分だけ、やってくる
            </a>
